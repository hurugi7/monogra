<x-guest-layout>
  ...
  <div class="mx-4 mt-4 border-solid border-gray-300 rounded-lg border shadow-sm bg-[#fcfcf2]">
    <div class="flex px-2 py-3 border-solid border-gray-300 border-b">
      ...
      <div class="font-bold">サブカテゴリ編集</div>
    </div>
    @if($errors->any())
    ...
    <form action="{{ route('have_sub_category.update', ['category' => $current_category_id, 'sub_category' => $sub_category->id]) }}" method="post" class="mx-8 my-3">
      ...
      <input type="text" class="block w-full p-4 mb-5 text-gray-900 border border-gray-300 rounded-lg bg-[#fcfcf2] sm:text-md focus:ring-ring-[#6c9cd2] focus:border-ring-[#6c9cd2]" name="sub_category_name" id="sub_category_name" value="{{ old('sub_category_name')?: $sub_category->sub_category_name }}">
